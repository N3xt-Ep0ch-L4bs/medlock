...
/**
 * Hook to check if a doctor has a DoctorProfile object in their wallet
 * Uses suix_getOwnedObjects to query for objects of type packageId::doctors::DoctorProfile
 */
export function useDoctorProfile(packageId: string): ProfileCheckResult {
  ...
  const checkProfile = useCallback(async () => {
    ...
    try {
      // Query for objects of type packageId::doctors::DoctorProfile
      const normalizedPackageId = packageId.startsWith('0x') ? packageId : `0x${packageId}`;
      const profileType = `${normalizedPackageId}::doctors::DoctorProfile`;
      ...
      console.error('Error checking for doctor profile:', err);
...
export function triggerDoctorProfileRefresh() {
