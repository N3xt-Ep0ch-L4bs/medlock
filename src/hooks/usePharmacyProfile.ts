...
/**
 * Hook to check if a pharmacy has a PharmacyProfile object in their wallet
 * Uses suix_getOwnedObjects to query for objects of type packageId::pharmacy::PharmacyProfile
 */
export function usePharmacyProfile(packageId: string): ProfileCheckResult {
  ...
  const checkProfile = useCallback(async () => {
    ...
    try {
      // Query for objects of type packageId::pharmacy::PharmacyProfile
      const normalizedPackageId = packageId.startsWith('0x') ? packageId : `0x${packageId}`;
      const profileType = `${normalizedPackageId}::pharmacy::PharmacyProfile`;
      ...
      console.error('Error checking for pharmacy profile:', err);
...
export function triggerPharmacyProfileRefresh() {
